...
export default function AlterarProduto() {
    ...
    const produto_idRecebido = location.state !== undefined ? location.state : null;
    console.log(produto_idRecebido)

    useEffect(() => {
        if (produto_idRecebido) {
            buscarProdutoPorproduto_id(produto_idRecebido);
        }
    }, [location.state]);
    ...
    async function buscarProdutoPorproduto_id(produto_id) {
        try {
            console.log('Buscando dados para o produto_id:', produto_id);

            const response = await fetch(`http://localhost:8080/produto?produtoId=${produto_id}`, {
                method: 'GET',
                headers: {
            ...
            setNomeProduto(responseData.nome);
            setCategoria(responseData.categoria);
            setMarca(responseData.marca);
            setPreco(responseData.preco);
            setDescricao(responseData.descricao);
            setLocEstoque(responseData.loc_estoque);
            setPeso(responseData.peso);

            setImagens(responseData.imagens.map((imagem) => {
                console.log('URL da Imagem:', imagem.url);
                return imagem.url;
            }));

            setVariacoes(() =>
                (responseData.variacoes || []).map((variacaoItem) => ({
    ...
    async function alterar(e) {
        ...
        try {
          ...
          console.log('Dados do Produto:', {
            ...
            preco: preco,
            descricao: descricao,
            loc_estoque: locEstoque === "" ? null : locEstoque,
            peso: peso,
          ...
          imagem.forEach((item) => {
            ...
            body.append("imagens", item);
          });
      
          console.log('Variações:', variacoes);

          const response = await fetch(`http://localhost:8080/produto?produtoId=${produto_idRecebido}`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          ...
          console.error('Erro ao realizar a alteração do produto:', error);
        }
      }

    const handleAddImagens = (event) => {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            reader.onloadend = () => {
                ...
                if (novasImagens.length === selectedFiles.length) {
                ...
                <div className="mainConteudo">
                    ...
                    <div className="formulario">
                        <form>
                            ...
                            <input id="preco" type="text" name="preco" value={preco} onChange={(e) => setPreco(e.target.value.replace(/[^0-9.]/g, ''))} />

                            <label htmlFor="descricao">Descrição:*</label>
                            <input id="descricao" type="text" name="descricao" value={descricao} onChange={(e) => setDescricao(e.target.value)}></input>

                            <label htmlFor="loc_estoque">Locação de estoque:</label>
                            <input id="loc_estoque" type="text" name="loc_estoque" value={locEstoque} onChange={(e) => setLocEstoque(e.target.value === "" ? null : e.target.value)}></input>

                            <label htmlFor="peso">Peso:*</label>
                            <input id="peso" type="text" name="peso" value={peso} onChange={(e) => { const inputPeso = e.target.value.replace(/[^0-9.]/g, ''); setPeso(inputPeso) }}></input>

                            <label htmlFor="imagem">• Imagem:*</label>
                            <div className="imagemProd">
                                <input type="file" id="imageInput" accept="image/*" multiple onChange={handleAddImagens} />
                                <div id="imageContainer">
                                    {imagem.map((imageUrl, index) => (
                                        <div key={index} className="image-item">
                                            <img src={imageUrl} alt={`Imagem ${index}`} />
                                            {/* <button type="button" onClick={(e) => deleteImage(index, e)}>
                            ...
                            <div className="variacoes">
                                ...
                                {variacoes.map((variacao, index) => (
                                    <div key={index} className="variacao">
                                        ...
                                        <input className='variacaoTexto' id={`tamanho-${index}`} type="text" name="tamanho" value={variacao.tamanho} placeholder="Tamanho" onChange={(event) => handleVariacaoChange(index, event)} disabled/>

                                        <label className='variacaoLabel' htmlFor={`cor-${index}`}>Cor:</label>
                                        <input className='variacaoTexto' id={`cor-${index}`} type="text" name="cor" value={variacao.cor} placeholder="Cor" onChange={(event) => handleVariacaoChange(index, event)} disabled/>

                                        <label className='variacaoLabel' htmlFor={`quantidade-${index}`}>Quantidade:</label>
                                        <input className='variacaoTexto' id={`quantidade-${index}`} type="text" name="quantidade" value={variacao.quantidade} placeholder="Quantidade" onChange={(event) => { const inputValue = event.target.value.replace(/[^0-9.]/g, ''); const sanitizedValue = inputValue.replace(/^-/, ''); handleVariacaoChange(index, { target: { name: 'quantidade', value: sanitizedValue } }); }} disabled/>
                                        {/* {variacoes.length > 1 && (
